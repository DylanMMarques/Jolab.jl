...
Axicon(α, axicon_medium, medium, frame; kwargs...) =
    Axicon(Float64, α, axicon_medium, medium, frame; kwargs...)
...
function _light_interaction!(
    ...
    FB<:MeshedBeam{TB,Backward,CB},
    FF<:MeshedBeam{TF,Forward,CF},
    F<:MeshedBeam{T,D,C},
    O<:Union{Fourier,Axicon},
} where {T,D,C,TF,CF,TB,CB}
    ...
    @inbounds for ind_in in eachindex(field_i.e)
        tmp_val +=
            t(
                ...
                axicon,
                β,
                centroid(field_i.mesh, ind_in),
                out_c,
                volume(field_i.mesh, ind_in) / field_i.mesh.spacing[3],
            ) * field_i.e[ind_in]
    end
...
function forward_backward_field(
    axicon::Union{Axicon,Fourier},
    field_i::F,
) where {F<:MeshedBeam{T,D,C}} where {T,D,C<:Union{X_Y_λ,NSX_NSY_λ}}
    (symbol, C_T) =
        F <: MeshedAngularSpectrum ? ((:x, :y), X_Y_λ) : ((:nsx, :nsy), NSX_NSY_λ)

    (haskey(axicon.solver, symbol[1]) && haskey(axicon.solver, symbol[2])) ||
        error("Solver must have cartesian coordinate (field x and y or nsx and nsy)")
    x = getfield(axicon.solver, symbol[1])
    y = getfield(axicon.solver, symbol[2])
    e_t = similar(
        field_i.e,
        Complex{T},
        (length(x), length(y), size(field_i.e)[3], size(field_i.e)[4]),
    )
    ...
    field_r =
        MeshedBeam{T,dir_r,C}(deepcopy(field_i.mesh), e_r, field_i.medium, field_i.frame)
...
function forward_backward_field(
    axicon::Union{Axicon,Fourier},
    field_i::F,
) where {F<:MeshedBeam{T,D,C,P}} where {T,D,C<:Union{R_θ_λ,NSR_NSθ_λ},P}
    (symbol, C_T) = F <: MeshedAngularSpectrum ? (:r, R_θ_λ) : (:nsr, NSR_NSθ_λ)

    haskey(axicon.solver, symbol) ||
        error("Solver must have radial coordinate (field r or nsr)")
    r = getfield(axicon.solver, symbol)
    e_t = similar(
        field_i.e,
        Complex{T},
        (length(r), size(field_i.e)[2], size(field_i.e)[3], size(field_i.e)[4]),
    )
    e_r = Zeros(T, size(field_i.e))
    ...
    (dir_r, dir_t) = reverse_if_backward(D, (Backward, Forward))
    field_r =
        MeshedBeam{T,dir_r,C,P}(deepcopy(field_i.mesh), e_r, field_i.medium, field_i.frame)

...
function field_after_lens(focal_len, nsx, nsy, ω, λ, medium)
    ...
    field = Jolab.AngularSpectrum(Jolab.Forward, grid_ang, Jolab.Gaussian(ω), medium, ReferenceFrame((0,0,0), (0,0,0)))
...
function field_after_lens(focal_len, x, y, ω, λ, medium)
    ...
    field = Jolab.SpatialBeam(Jolab.Forward, grid_spatial, Jolab.Gaussian(ω), medium, ReferenceFrame((0,0,0), (0,0,0)))
    (rfield, tfield) = light_interaction(lens, field)
    return maximum(abs, tfield.e)
end
...
function test_autodiff(λ)
    ...
    field = Jolab.SpatialBeam(Jolab.Forward, grid_field, Jolab.Gaussian(1E-3), Medium(1), ReferenceFrame((0,0,0), (0,0,0)))
...
ad_diff = autodiff(Enzyme.Forward, Jolab.rtss, Duplicated, Const(stack_test), Const(Forward), Const(0.1), Duplicated(1550E-9, 1.0))[1]
...
ad_diff = autodiff(Enzyme.Forward, Jolab.rtss, Duplicated, Const(stack_test), Const(Forward), Duplicated(0.1, 1.0), Const(1550E-9))[1]

...
beam_space_analytical = Jolab.SpatialBeam(
    Float64,
    Forward,
    grid_spatial,
...
ang_spe_analitical = Jolab.AngularSpectrum(
    Float64,
    Forward,
    grid_angular,
    Jolab.Gaussian(50E-6),
...
grid_spatial_shift =
    Jolab.CartesianGrid(Jolab.X_Y_λ, x .+ x[512+124], y .+ y[512+124], λ_shift)
space_t = Jolab.SpatialBeam(
    Float64,
    Forward,
    grid_spatial_shift,
    Jolab.Gaussian(50E-6),
...
@test findmax(abs, beam_space_trans.e)[2] ==
      (findmax(abs, space_t.e)[2] + CartesianIndex(123, 123, 0, 0))

e = rand(ComplexF64, length(x), length(x), 1, 3)
grid_spatial_e = Jolab.CartesianGrid(Jolab.X_Y_λ, x, x, 1550E-9)
beam = Jolab.MeshedBeam{Float64,Forward,Jolab.X_Y_λ,Jolab.PolarizationXYZ}(
    grid_spatial_e,
    e,
    Medium(1),
    ReferenceFrame((0, 0, 0), (0, 0, 0)),

...
nst = range(0, 2π, length = 1000)[1:end-1]
λ = 1550E-9
grid_ang_radial = Jolab.CylindricalGrid(Jolab.NSR_NSθ_λ, nsr, nst, λ)
field = Jolab.AngularSpectrum(Jolab.Forward, grid_ang_radial, Jolab.Gaussian(10E-6), Medium(2), ReferenceFrame((0,0,0), (0,0,0)))
...
nst = range(0, 2π, length = 1000)[1:end-1]
grid_ang_radial = Jolab.CylindricalGrid(Jolab.NSR_NSθ_λ, nsr, nst, λ)
field = Jolab.AngularSpectrum(Jolab.Backward, grid_ang_radial, Jolab.Gaussian(10E-6), Medium(2), ReferenceFrame((0,0,100E-9), (0,0,0E-9)))
mls = DielectricStack([Medium(2), Medium(2), Medium(2)], [100E-9], ReferenceFrame((0,0,0), (0,0,0)))
field_mls = light_interaction(mls, field)
prop = Propagation((ReferenceFrame((0,0,0), (0,0,0)), ReferenceFrame((0.0,0.0,100E-9), (0,0,0))), Medium(2))
fields_prop = light_interaction(prop, field)
...
nst = range(0, 2π, length = 1000)[1:end-1]
grid_ang_radial = Jolab.CylindricalGrid(Jolab.NSR_NSθ_λ, nsr, nst, λ)
field = Jolab.AngularSpectrum(Jolab.Forward, grid_ang_radial, Jolab.Gaussian(10E-6), Medium(2+im), ReferenceFrame((0,0,0), (0,0,0)))

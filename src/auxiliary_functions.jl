...
function check_same_definition(
    mode1::M1,
    mode2::M2,
) where {M2<:AbstractFieldMode{<:Any,D2},M1<:AbstractFieldMode{<:Any,D1}} where {D1,D2}
...
function check_same_definition(
    beam1::MeshedBeam{T1,D,C},
    beam2::MeshedBeam{T2,D,C},
) where {T1,T2,D,C}
...
function overlap_integral(array1::AbstractArray, array2::AbstractArray, mesh::Domain)
    ...
    mapreduce(
        (i) -> array1[i] * conj(array2[i]) * integration_space(mesh, i),
        +,
        eachindex(mesh),
    )
end
...
function overlap_integral(array::AbstractArray, f2::Function, mesh::Domain)
    ...
    mapreduce(
        (i) -> array[i] * conj(f2(centroid(mesh, i))) * integration_space(mesh, i),
        +,
        eachindex(mesh),
    )
end
...
function overlap_integral(f2::Function, array::AbstractArray, mesh::Domain)
    ...
    mapreduce(
        (i) -> conj(array[i]) * f2(centroid(mesh, index)) * integration_space(mesh, i),
        +,
        eachindex(mesh),
    )
end
...
function get_ranges(
    grid::Union{<:CylindricalGrid{<:Any,N},<:CartesianGrid{<:Any,N}},
) where {N}
    function f(i)
        ...
        if isone(length_i)
            ...
            LinRange(
                grid.origin[i],
                grid.origin[i] + (length_i-1) * grid.spacing[i],
                length_i,
            )
        end
    end

...
x = LinRange(-0.1, 0.15, 100)
y = LinRange(-0.2, 0.3, 100)
z = LinRange(-4, 5, 100)
xyz = (x, y, z)
...
@test_throws ArgumentError Jolab.CylindricalGrid(
    (10, 10, 10),
    Jolab.R_θ_λ(0.0, -0.1, 0.0),
    (0.1, 2π / 10, 0.1),
)
...
@test Jolab.centroid(grid, CartesianIndex(10, 1, 10)) == [0.9, 0.0, 0.9]
...
@test_throws ArgumentError Jolab.CartesianGrid(
    Jolab.R_θ_λ,
    range(0, 1),
    range(0, 2π),
    1550E-9,
...
@test Jolab.same_coordinate_type(Jolab.X_Y_Z(1.0, 2.0, 3.0), Jolab.X_Y_Z(4.0, 5.0, 6.0)) ==
      true
@test Jolab.same_coordinate_type(
    Jolab.X_Y_λ(1.0, 2.0, 1550E-9),
    Jolab.X_Y_λ(4.0, 5.0, 1550E-9),
) == true
@test Jolab.same_coordinate_type(Jolab.X_Y_Z(1.0, 2.0, 3.0), Jolab.X_Y_λ(1.0, 2.0, 3.0)) ==
      false

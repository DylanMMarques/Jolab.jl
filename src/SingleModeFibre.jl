...
struct_type(::Type{SingleModeProfile{T}}) where {T} =
    SVector{1,GaussianMode{T,Bothway,Complex{T}}}
...
SingleModeFibre(mfd, length, media, frames) =
    SingleModeFibre(Float64, mfd, length, media, frames)
...
function check_input_field(
    fib::Fibre{<:Any,<:SingleModeProfile},
    field::MeshedBeam{<:Any,D,<:Any,P},
) where {D,P<:AbstractPolarization}
    (frame, medium) =
        D == Forward ? (fib.frames[1], fib.media[1]) : (fib.frames[2], fib.media[2])
    code = zero(UInt64)
...
function forward_backward_field(
    ...
    mode_vec = StructVector{GaussianMode{T,D,Complex{T}}}((
        modes_e,
        modes_sigma,
        modes_wavelength,
        frames,

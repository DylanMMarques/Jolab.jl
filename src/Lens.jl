struct Lens{T,F,N,M1<:Medium{T},M2<:Medium{T}} <: AbstractOpticalElement{T}
    ...
    function Lens(
        ...
        any(is_complex_medium, media) && throw(
            ArgumentError(
                "A lens cannot be in a medium with absortion. Use media with real refractive index.",
            ),
        )

        frame_1 =
            frame.origin + _RotXYZ(frame.direction) * X_Y_Z(zero(T), zero(T), -focal_length)
        frame_2 =
            frame.origin + _RotXYZ(frame.direction) * X_Y_Z(zero(T), zero(T), focal_length)
        new{T,F,N,M1,M2}(
            focal_length,
            numerical_aperture,
            media,
            map(o -> ReferenceFrame(o, frame.direction), (frame_1, frame_2)),
        )
    end
end
export Lens

Lens(focal_length, numerical_aperture, media, frame) =
    Lens(Float64, focal_length, numerical_aperture, media, frame)

@inbounds function _light_interaction!(
    field_b::MeshedBeam{<:Any,Backward},
    field_f::MeshedBeam{<:Any,Forward},
    lens::Lens,
    field_i::MeshedBeam{T,D,C,PolarizationScalar},
) where {T,D,C}
...
@inbounds function _light_interaction!(
    ...
    field_f::MeshedBeam{<:Any,Forward},
    lens::Lens,
    field_i::MeshedBeam{T,D,C,PolarizationXYZ},
) where {T,D,C<:SpatialCoords}
    (field_r, field_t) = reverse_if_backward(D, (field_b, field_f))
    fill!(field_r.e, 0)
...
function _ScatteringMatrix(
    ...
    field_f::MeshedBeam,
    comp::Lens,
    field_i::MeshedBeam{T,D,C,P},
) where {T,D,C,P<:PolarizationScalar}
    (field_r, field_t) = reverse_if_backward(D, (field_b, field_f))
...
function forward_backward_field(lens::Lens, field_i::MeshedBeam{T,D,C,P}) where {T,D,C,P}
    ...
    e_t = similar(
        field_i.e,
        Complex{T},
        (size(field_i.mesh)[1:3]..., number_components(P_out)),
    )
    ...
    P_out = get_polarization_type(Lens, P)
    field_t = MeshedBeam{T,dir_t,get_transmitted_coord_type(Lens, C),P_out}(
        mesh,
        e_t,
        lens.mat[2],
        frame_t,
...
function check_output_fields(field_b, field_f, comp::Lens, field_i::MeshedAngularSpectrum)
    ...
    msg = ""
    (field_r.mesh == field_i.mesh) ||
        (msg *= "Meshes of the reflected field must be the same as the input field.\n")
    (field_r.medium == field_i.medium) ||
        (msg *= "Medium of the reflected field must be the same as the input field.\n")
    (n_r == field_r.medium) || (
        msg *= "Medium of the reflected field must be the same as the lens medium which the field is incident upon.\n"
    )
    (n_t == field_t.medium) || (
        msg *= "Medium of the transmitted field must be the same as the lens medium which the field is transmitted to.\n"
    )
    f = comp.focal_length
    field_t.mesh == Scale(f / n_t, f / n_t, 1)(field_i.mesh) || (
        msg *= "Meshes of the transmitted field must be scaled by the focal length and refractive index.\n"
    )

    (ref_r, ref_t) = reverse_if_backward(D, comp.frames)
    (field_r.frame == ref_r) || (
        msg *= "Reference frame of the reflected field must be the same as the lens reference frame.\n"
    )
    (field_t.frame == ref_t) || (
        msg *= "Reference frame of the transmitted field must be the same as the lens reference frame.\n"
    )
...
function check_input_field(lens::Lens, field_i::MeshedBeam{T,D}) where {D,T}
    ...
    (field_i.medium ≈ (D == Forward ? first : last)(lens.mat)) ||

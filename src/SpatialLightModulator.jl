...
function SpatialLightModulator_diskaperture(
    ::Type{T},
    internal_radius,
    outer_radius,
    media,
...
function reflection_transmission_function(
    slm::SpatialLightModulator{<:Any,C},
    ::Type{C},
) where {C}
    slm.r, slm.t
end,
...
function _light_interaction!(
    ...
    broadcast!(tmp_vec, eachindex(field_i.e)) do i
        ...
        (
            slm_r(coord[1], coord[2], coord[3]) * field_i.e[i],
            slm_t(coord[1], coord[2], coord[3]) * field_i.e[i],
        )
    end

function test_cartesian_with_radialsymmetric(ω, comp, medium, sampling_rad, sampling_cart; rtol = 1E-9)
    ...
    radial_i = Jolab.AngularSpectrum(Jolab.Forward, grid_rad, Jolab.Gaussian(ω), medium, ReferenceFrame((0,0,0), (0,0,0)))
    nsx = range(-0.5, 0.5, length = sampling_cart)
    grid_ang = Jolab.CartesianGrid(Jolab.NSX_NSY_λ, nsx, nsx, λ)
    cart_i = Jolab.AngularSpectrum(Jolab.Forward, grid_ang, Jolab.Gaussian(ω), medium, ReferenceFrame((0,0,0), (0,0,0)))

...
struct FourierFFTSolver{F1,F2,F3,P<:AbstractFFTs.Plan,P2<:AbstractFFTs.Plan,A} <:
       AbstractSolver
    field_i::F1
    field_b::F2
    field_f::F3
...
function _light_interaction!(
    ...
    fourier::FourierFFTSolver,
    field_i::MeshedBeam{T,D,C,P},
) where {T,D,C<:NSX_NSY_λ,P<:Union{PolarizationScalar,PolarizationXYZ}}
...
function _light_interaction!(
    ...
    field_t.e .*=
        exp.(-(im * 2π / λ) .* (first(x) .* nsx .+ first(y) .* (nsy)')) .*
        (step(x) * step(y) / (2π)^2)
...
function check_input_field(f::FourierFFT, field_i::MeshedBeam{T,D,C}) where {T,D,C}
    if !(C <: X_Y_λ || C <: NSX_NSY_λ)
        throw(
            ArgumentError("FourierFFT only supports X_Y_λ and NSX_NSY_λ coordinate types"),
        )
    end
    zero(UInt64)
end
...
function forward_backward_field(
    ...
    field_t = MeshedBeam{T,dir_t,get_transmitted_coord_type(FourierFFT, C),P}(
        mesh,
        e_t,
        field_i.medium,
        field_i.frame,

struct MeshedBeam{
    ...
    V,
    E<:AbstractArray{<:RealOrComplex{T},4},
    M<:Medium{T,<:RealOrComplex{T}},
    T2<:RealOrComplex{T},
} <: AbstractField{T,D}
    mesh::V
    e::E
    ...
    function MeshedBeam{T,D,C,P}(
        mesh::V,
        e::E,
        medium::Medium{<:Any,M},
        frame::ReferenceFrame,
    ) where {T,V,E<:AbstractArray{T2},D,C,P,M} where {T2}
...
_electric_field_f(gauss::Gaussian, coord::R_θ_λ) =
    exp(-(coord[1] * 2 / gauss.ω)^2) * 2 * √(2 / π) / gauss.ω
_electric_field_f(gauss::Gaussian, coord::X_Y_λ) = _electric_field_f(gauss, R_θ_λ(coord))

_electric_field_f(gauss::Gaussian, coord::NSR_NSθ_λ) =
    exp(-gauss.ω^2 * π^2 * coord[1]^2 / 4 / coord[3]^2) * gauss.ω / (4 * √2 * π^(3/2))
_electric_field_f(gauss::Gaussian, coord::NSX_NSY_λ) =
    _electric_field_f(gauss, NSR_NSθ_λ(coord))
...
function _SpatialBeam(
    ...
    medium::Medium,
    frame::ReferenceFrame,
) where {C<:SpatialCoords,D<:AbstractDirection,T<:Real}
...
function _AngularSpectrum(
    ...
    medium::Medium,
    frame::ReferenceFrame,
) where {C<:AngularSpectrumCoords,D<:AbstractDirection,T<:Real}
    e_reshaped = reshape(e, size(grid)[1:3]..., 1)
    MeshedBeam{T,D,C,PolarizationScalar}(grid, e_reshaped, medium, frame)
end,
function _AngularSpectrum(
...
function Base.isapprox(
    beam1::MeshedBeam{T1,D,C1},
    beam2::MeshedBeam{T2,D,C2};
    kwargs...,
) where {T1,T2,D,C1,C2}
...
function translate_referenceframe(
    beam::MeshedAngularSpectrum{T,D,C},
    new_origin::X_Y_Z,
) where {T,D,C}
...
function nsz_nocomplex(n, nsx, nsy)
    tmp = (n^2 - nsx^2 - nsy^2)
    signbit(real(tmp)) &&
        throw(ArgumentError("The reference frame of evasnescent waves cannot be rotated."))
    return √tmp
end
...
translate_referenceframe(
    pw::Union{AbstractFieldMode{T},MeshedBeam{T}},
    new_origin,
) where {T} = translate_referenceframe(pw, X_Y_Z(new_origin))
...
function change_polarization_basis(
    field::MeshedBeam{T,D,C,PolarizationSP},
    ::Type{PolarizationXYZ},
) where {T,D,C<:AngularSpectrumCoords}
    ...
    return MeshedBeam{T,D,C,PolarizationXYZ}(
        field.mesh,
        field_new,
        field.medium,
        field.frame,
...
function _unchecked_add!(
    field1::MeshedBeam{T,D,C,P},
    field2::MeshedBeam{T,D,C,P},
) where {T,D,C,P}
    field1.e .+= field2.e
    field1
end

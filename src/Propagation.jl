struct Propagation{T,R<:ReferenceFrame{T},M<:Medium{T}} <: AbstractOpticalElement{T}
    ...
    function Propagation(
        ...
        frames::NTuple{2,R},
        medium::M,
    ) where {T,R<:ReferenceFrame{T},M<:Medium{T}}
...
t(prop::Propagation, ::Type{D}, medium, rΔpos::X_Y_Z, coord::NSR_NSθ_λ) where {D} =
    t(prop, D, medium, rΔpos, NSX_NSY_λ(coord))
...
function forward_backward_field(
    ...
    frame = D == Forward ? prop.frames[2] : prop.frames[1]
    field_r = MeshedAngularSpectrum{T,!D,C,P}(
        field_i.mesh,
        Zeros(T, size(field_i.e)),
        field_i.medium,
        field_i.frame,
    )
    field_t = MeshedAngularSpectrum{T,D,C,P}(
        field_i.mesh,
        similar(field_i.e, Complex{T}),
        field_i.medium,
        frame,
    )

struct RoughInterface{T,T2<:RealOrComplex{T},F} <: AbstractOpticalElement{T}
    ...
    ) where {T,T2<:RealOrComplex{T},F,N<:Tuple{Medium{T2},Medium{T2}}}
...
struct RoughInterfaceRRSolver{F1,F2,F3,Y,X,Z,P} <: AbstractSolver
    ...
    tmp_array::X
    tmp_array_2::X
    function RoughInterfaceRRSolver(
        field_b::F2,
        field_f::F3,
        field_i::F1,
        r12::Y,
        ...
        tmp_array,
        tmp_array_2,
    ) where {F1,F2,F3,Y,X,Z,P}
        new{F1,F2,F3,Y,X,Z,P}(
            field_b,
            field_f,
            field_i,
            r12,
...
function RoughInterfaceRRSolver(
    comp::RoughInterface,
    field_i::MeshedAngularSpectrum{T,D,P},
) where {T,D,P}
    ...
    fftconst =
        step(x) * step(y) / 4π^2 * length(nsx) * length(nsy) * step(nsx) * step(nsy) * k^2
...
function _light_interaction!(
    ...
    solver::RoughInterfaceRRSolver,
    field_i::MeshedBeam{T,D,<:NSX_NSY_λ,P},
) where {T,D,P<:PolarizationScalar}
...
function forward_backward_field(
    comp::RoughInterfaceRRSolver,
    field_i::MeshedAngularSpectrum,
)

...
function same_coordinate_type(
    c1::C1,
    c2::C2,
) where {C1<:AbstractCoordinate,C2<:AbstractCoordinate}
...
function CartesianGrid(
    ::Type{C},
    x::AbstractRangeOrNumber,
    y::AbstractRangeOrNumber,
    z::AbstractRangeOrNumber,
) where {C}
    C <: Union{R_θ_λ,NSR_NSθ_λ} && throw(
        ArgumentError(
            "CartesianGrid does not support cylindrical coordinates ($C). Use CylindricalGrid instead.",
        ),
    )
    xyz = (x, y, z)
    lengths = length.(xyz)
...
area(grid::CartesianGrid{C,2}, ind::Union{Integer,CartesianIndex{2}}) where {C} =
    prod(grid.spacing)
volume(grid::CartesianGrid{C,3}, ind::Union{Integer,CartesianIndex{3}}) where {C} =
    prod(grid.spacing)
...
function Base.checkbounds(grid::Domain{C,Dim}, ind::CartesianIndex{Dim}) where {C,Dim}
    all(i -> 1 <= i[1] <= i[2], Iterators.zip(ind.I, grid.lengths)) ||
        throw(BoundsError(grid, ind))
end,
...
struct CylindricalGrid{C,Dim,T} <: Domain{C,Dim,T}
    ...
    function CylindricalGrid(
        ...
        0 <= θ_start <= 2π || throw(
            ArgumentError(
                "CylindricalGrid requires the angular origin to be within [0, 2π].",
            ),
        )
        0 <= θ_stop <= 2π || throw(
            ArgumentError(
                "CylindricalGrid requires the angular range to stay within [0, 2π].",
            ),
        )
        new{C,Dim,T}(spacing, origin, lengths)
    end
...
function CylindricalGrid(
    ::Type{C},
    r::AbstractRangeOrNumber,
    θ::AbstractRangeOrNumber,
    z::AbstractRangeOrNumber,
) where {C}
    C <: Union{X_Y_λ,NSX_NSY_λ} && throw(
        ArgumentError(
            "CylindricalGrid does not support Cartesian coordinates ($C). Use CartesianGrid instead.",
        ),
    )
    rθz = (r, θ, z)
    lengths = length.(rθz)
...
function Base.isapprox(
    grid::CylindricalGrid{C1,3},
    grid2::CylindricalGrid{C2,3};
    kwargs...,
) where {C1<:AbstractCoordinate,C2<:AbstractCoordinate}
    same_coordinate_type(C1, C2) &&
...
function Base.isapprox(
    grid::CartesianGrid{C1,3},
    grid2::CartesianGrid{C2,3};
    kwargs...,
) where {C1<:AbstractCoordinate,C2<:AbstractCoordinate}
    same_coordinate_type(C1, C2) &&
        all(isapprox.(grid.spacing, grid2.spacing; kwargs...)) &&
...
function (scale::Scale{N})(mesh::CylindricalGrid{C,N}) where {N,C}
    CylindricalGrid(
        mesh.lengths,
        C(mesh.origin .* scale.factors),
        mesh.spacing .* scale.factors,
    )
end
    ...
    eval(
        quote
            function Base.view(
                ...
                $domain(
                    (grid.lengths[1:2]..., length(ind)),
                    C(
                        grid.origin[1:2]...,
                        grid.origin[3] + grid.spacing[3] * (first(ind) - 1),
                    ),

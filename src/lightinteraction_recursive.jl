function _lightinteraction_recursivegridded!(
    fields_l,
    fields_r,
    coefs,
    fieldi::AbstractField{T,D};
    rtol = 1E-3::Real,
    printBool = False,
    ...
    while true
        ...
        if sum(int_l) + sum(int_r) > 10 * initial_int || now_int > 10 * min_int
            if printBool == true
                ...
                println(
                    "lightinteraction_recursivegridded is not converging. Current number of iterations:",
                    i,
                )
            end
            converge = false
        ...
        if i > maximum_iterations && printBool == true
            println(
                "Max number of iterations achieved. Current light intensity:",
                (sum(int_l) + sum(int_r)) / initial_int,
            )
            converge = false
            break
        end
        if printBool == true
            ...
            println(
                ...
                sum(view(int_l, 2:sizeL)) + sum(view(int_r, 1:(sizeL-1))),
                " < ",
                rtol,
            )
        end
...
function lightinteraction_recursivegridded(
    comp,
    fieldi::AbstractField{T,D};
    kwargs...,
) where {T,D}
    iter = D == Forward ? identity : Iterators.reverse
    field_transmitted(coef, field_i) =
        last(reverse_if_backward(D, forward_backward_field(coef, field_i)))
    ...
    (fields_backward, fields_forward) =
        reverse_if_backward(D, (fields_reflected, fields_transmitted))
    _lightinteraction_recursivegridded!(
        fields_backward,
        fields_forward,
        solvers,
        fieldi;
